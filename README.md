...
Python code has been implemented and tested with Python 3.11.9. The Script_Intaller.py updates the python to newest version and installs and updates the required packages. The scripts use the following Python packages:
- numpy 1.26.4
- openpyxl 3.1.5
- matplotlib 3.9.1
- scipy 1.14.0
...
- Data_files should contain the raw data files that will be analysed by the scripts. Three analyzed raw data files (Test_EPD_96wp.xlsx, Test_EPD_384wp.xlsx and Test_DORA_384wp.xlsx) are provided with information    sheet on how the raw data files were run with scripts. Test_EPD_96wp.xlsx shows the formating of data that works for both EPDScript_96.py and DORAScript_96.py scripts.
