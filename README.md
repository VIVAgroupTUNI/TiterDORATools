...
Supplementary code for paper Quantitative enterovirus infectivity titration and dose response analyses using cell-culture based methods and python code assisted data analysis.
...
- Data_files should contain the raw data files that will be analysed by the scripts. Two analyzed raw data files (EPD and DORA) are provided with information sheet on how the raw data files were run with scripts.
- Master_file should contain the master file where EPD results are stored. Example master file is provided, see further instructions from script tutorial as explained above. 
