...
Summary:
This script is designed to process and analyze data from 96-wellplate experiments. The primary functionalities include
...
class Group:
    ...
    def get_normalized(self):
        ...
        # Return the normalized and rescaled data
        return {
            "topline": topline,
            "baseline": baseline,
...
def info():
    ...
    try:
        ...
        print_info("Notes:", [
            ...
            "Should the user provide an incorrect value for a group during its definition, they may reset defining the current group and start anew by giving a command of \"...\" instead of the actual asked value."
...
def print_status(status_string, leading_line_break=True, tailing_line_break=False):
    ...
    try:
        time.sleep(DELAY)
        text = f"DORA-96 SCRIPT {status_string.upper()}"
        # Middle line padding
        asterisks = "*" * 3
        spaces = " " * 5
...
def validate_absorbance(absorbance, row_char, col_num):
    ...
    return False, f'Absorbance value "{absorbance}" of well {row_char}{col_num} was invalid. Expected a number.'
...
def validate_parallel(sheet, row, first_col):
    ...
    for col in range(first_col, WELLS_IN_A_PARALLEL + first_col):
        ...
        is_valid, validation_result = validate_absorbance(cell_value, row, col)
        if not is_valid:
            return False, validation_result
    return True, None
    ...
    for row in range(1, sheet.max_row + 1):
        for col in range(START_COL, END_COL + 1):
            ...
            for check_row in range(row, WELLPLATE_ROWS + row):
                is_valid, validation_result = validate_parallel(sheet, check_row, col)
                if not is_valid:
                    continue
                valid_row_count += 1
...
def define_groups():
    ...
    while True:
        try:
            ...
            # Based on the title, determine wether an actual analysis type or filler
...
def get_wellplate_data(wellplate, tissues):
    ...
    for i, row in enumerate(wellplate):
        ...
        for j, well in enumerate(row):
            ...
            if absorbance:
                # If the well contains a value, increment the total found absorbance values coutner
                realized_total += 1
...
def extract_wellplate_data(data_sheet, groups):
    ...
    wellplate_info = find_wellplate(data_sheet)
    # If no wellplate was found then the sheet contains no more plates
    if not wellplate_info:
        exit_by_error(f"Could not locate a wellplate in data sheet '{data_sheet.title}'.")
    ...
    print_progress(f"Wellplate extracted from [{chr(64+start_col)}{start_row}, {chr(64+end_col)}{end_row}].")
    # Get the absorbance values from the wellplate segment
    wellplate_data = get_wellplate_data(wellplate_segment, groups)
    print_progress(f'Wellplate data ({len(wellplate_data)} values) was valid.')
    return wellplate_data
...
def get_next_key(key):
    ...
    segment_start = max([start_col for start_col in PARALLEL_FIRST_COLS if start_col <= col_num])
    ...
    if segment_end <= next_col_num:
        # Reset the col number
        next_col_num = segment_start
        # Determine the next row character
        next_row_char = chr(row_char_ascii + 1)
            ...
            for parallel in sample.parallels:
                while not parallel.is_filled(): # Ensure this parallel is fully filled before moving to the next one
                    if not current_key:
                        ...
                        exit_by_error(f'The wellplate did not contain enough absorbance data to integrate Group {group.ordinal}, sample "{sample.label}".')
...
def calculate_wellplate_statistics(wellplate_data):
    """Function to calculate the statistics of the 96-wellplate."""
    # Constants for rows and columns
    ROW_CHARS = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']
    NEG_CTRL_COL_NUMS = [1, 2]
    VIR_CTRL_COL_NUM = 12
    # Lists to store actual absorbance values for the wellplate
    wellplate_neg_ctrl_values = []
    wellplate_vir_ctrl_values = []
    # Loop through the rows of the wellplate
    for row_char in ROW_CHARS:
        # Collect negative control values
        for neg_ctrl_col in NEG_CTRL_COL_NUMS:
            value = wellplate_data.get((row_char, neg_ctrl_col), None)
            if value:
                wellplate_neg_ctrl_values.append(value)
        # Collect viral control values
        value = wellplate_data.get((row_char, VIR_CTRL_COL_NUM), None)
        if value:
            wellplate_vir_ctrl_values.append(value)
    # Calculate and store statistics for the wellplate
    wellplate_statistics = {
        'neg_ctrl_avg': mean(wellplate_neg_ctrl_values) if wellplate_neg_ctrl_values else "None",
        'neg_ctrl_avg_half': mean(wellplate_neg_ctrl_values) / 2 if wellplate_neg_ctrl_values else "None",    
        'neg_ctrl_stdev': stdev(wellplate_neg_ctrl_values) if 1 < len(wellplate_neg_ctrl_values) else "None",
        'vir_ctrl_avg': mean(wellplate_vir_ctrl_values) if wellplate_vir_ctrl_values else "None",
        'vir_ctrl_stdev': stdev(wellplate_vir_ctrl_values) if 1 < len(wellplate_vir_ctrl_values) else "None"
    }
    # Return wellplate statistics
    return wellplate_statistics
...
def write_wellplate_statistics(statistics_sheet, wellplate_statistics, write_row, write_col):
    ...
    wellplate_statistics (dict): The dictionary containing 96-wellplate statistics of averages and standard deviations.
    ...
    statistics_sheet.cell(row=write_row, column=write_col).value = "96-wellplate"
    ...
    statistics_sheet.cell(row=write_row + 2, column=write_col + 1).value = wellplate_statistics['neg_ctrl_avg']
    ...
    statistics_sheet.cell(row=write_row + 2, column=write_col + 2).value = wellplate_statistics['neg_ctrl_avg_half']
    ...
    statistics_sheet.cell(row=write_row + 2, column=write_col + 3).value = wellplate_statistics['neg_ctrl_stdev']
    ...
    statistics_sheet.cell(row=write_row + 3, column=write_col + 1).value = wellplate_statistics['vir_ctrl_avg']
    ...
    statistics_sheet.cell(row=write_row + 3, column=write_col + 3).value = wellplate_statistics['vir_ctrl_stdev']
    ...
    wellplate_col_char = get_col_char(write_col)
    col_width = get_col_width(statistics_sheet, wellplate_col_char)
    statistics_sheet.column_dimensions[wellplate_col_char].width = col_width
    # Return the next row and column values
    return write_row + 5, write_col


def write_statistics_data(statistics_sheet, group, group_statistics, write_row, write_col):
    """Function to write the statistics of an group to the statistics sheet.
...
def write_basic_data(statistics_sheet, group, write_row, write_col):
    """Function to write the basic data of a group to the statistics sheet.
...
def statistics_to_data_file(data_file, wellplate_data, groups):
    ...
    wellplate_statistics = calculate_wellplate_statistics(wellplate_data)
    ...
    write_wellplate_statistics(statistics_sheet, wellplate_statistics, 1, 1)
    print_progress(f'Statistics of 96-wellplate written to sheet "{statistics_sheet.title}".')
    for group in groups:
