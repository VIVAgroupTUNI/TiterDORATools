...
def info():
    ...
    try:
        ...
        print_info("1) Evaluating Environment:", [
            ...
            "It checks for the existence of 'master_files' and 'data_files' directories.",
        ...
        print_info("3) Selecting Master File:", [
            "Next, the user will be prompted to select a master file from the 'master_files' directory.",
...
def print_status(status_string, leading_line_break=True, tailing_line_break=False):
    ...
    try:
        ...
        asterisks = "*" * 3
        spaces = " " * 5
        text = f"EPD-96 SCRIPT {status_string.upper()}"
...
def find_wellplate_start_primary(sheet):
...
def find_wellplate_start_secondary(sheet):
    """Function to find the starting row and column for the wellplate data by looking for numeric values in the first column.
    
    Returns:
    tuple: A tuple containing the starting row and column, or None if not found.
    """
    START_COL = 1    
    for row in range(1, sheet.max_row + 1):
        cell_value = sheet.cell(row=row, column=START_COL).value
        if isinstance(cell_value, (int, float)):
            # Check if there are enough numeric values to cover all parallel wells
            valid_start = True
            for offset in range(WELLS_IN_A_PARALLEL):
                next_cell_value = sheet.cell(row=row, column=START_COL + offset).value
                if not isinstance(next_cell_value, (int, float)):
                    valid_start = False
                    break
            if valid_start:
...
def define_tissues():
    ...
    for i in range(2):
        ...
        for j, tissue_title in enumerate(tissue_titles):
            print_choice(j+1, tissue_title)
        ...
        for j in range(sample_count):
            tube_label = get_user_input(f"Please enter tube label for sample {j+1} ({tissue_title}{ordinal})", validate_non_empty, False)
        ...
        yes_no_value = get_user_input('Would you like to add a second tissue? (yes/no)', validate_yes_no)
        if yes_no_value in ['n', 'no']:
            break
    return tissues


def get_wellplate_data_primary(wellplate, tissues):
    """Creates a dictionary of absorbance values from the wellplate segment with automatically detected row chars and col nums.
...
def get_wellplate_data_secondary(wellplate, tissues):
    ...
    for i, row in enumerate(wellplate):
        ...
        is_valid, validation_result = validate_row_char(chr(64+i+1))
        if not is_valid:
            exit_by_error(validation_result)
        row_char = validation_result
        for j, well in enumerate(row):
            # Manually create the column number based on the well
            is_valid, validation_result = validate_col_num(j+1)
            if not is_valid:
                exit_by_error(validation_result)
            col_num = validation_result
            absorbance = well.value
    ...
    if realized_total < required_total:
        # If there weren't enough data in the wellplate to cover the requirements, terminate the script
        exit_by_error(f"The wellplate did not contain enough absorbance values. The defined samples required a total of {required_total} but only {realized_total} were found.")
    elif required_total < realized_total:
        # If there was too much data in the wellplate that exceeded the requirements, terminate the script
        exit_by_error(f"The wellplate contained too many absorbance values. The defined samples required a total of {required_total} but {realized_total} were found.")
    print_progress(f'Wellplate data ({len(wellplate_data)} values) was valid.')
    return wellplate_data


def extract_wellplate_primary(data_sheet, start_row, start_col):
    """Extracts wellplate data using the primary method and with row chars and col nums."""
    ...
    # Extract the wellplate
    wellplate = data_sheet[first_cell:last_cell]
    return wellplate


def extract_wellplate_secondary(data_sheet, start_row, start_col):
    """Extracts wellplate data using the secondary method and without row chars and col nums."""
    print_progress(f"Wellplate located in segment [{chr(64 + start_col)}{start_row}, {chr(64 + start_col + WELLPLATE_COLS - 1)}{start_row + WELLPLATE_ROWS - 1}].")
    # Determine the coordinates of the first and last cell of the segment
    first_cell = data_sheet.cell(row=start_row, column=start_col).coordinate
    last_cell = data_sheet.cell(row=start_row + WELLPLATE_ROWS - 1, column=start_col + WELLPLATE_COLS - 1).coordinate
    # Extract the wellplate
    wellplate = data_sheet[first_cell:last_cell]
    return wellplate
...
def extract_wellplate_data(data_sheet, tissues):
    ...
    # Try to extract wellplate data using the primary method
    start_row, start_col = find_wellplate_start_primary(data_sheet) # Find the starting row and column of the wellplate
    if isinstance(start_row, int):
        wellplate = extract_wellplate_primary(data_sheet, start_row, start_col) # Extract the wellplate segment from the sheet
        wellplate_data = get_wellplate_data_primary(wellplate, tissues) # Get the absorbance values from the wellplate segment
        return wellplate_data
    # If primary method fails, try the secondary method
    print_progress("Couldn't find row characters or column numbers, looking for data directly.")
    start_row, start_col = find_wellplate_start_secondary(data_sheet)
    if isinstance(start_row, int):
        wellplate = extract_wellplate_secondary(data_sheet, start_row, start_col)
        wellplate_data = get_wellplate_data_secondary(wellplate, tissues)
        return wellplate_data
    exit_by_error(f'Could not locate wellplate in data sheet "{data_sheet.title}".')
...
def get_next_key(key):
    ...
    """
    ASCII_A = ord('A')
    ASCII_RESET = ASCII_A + WELLPLATE_ROWS
    ...
    if segment_end <= next_col_num_int:
        ...
        next_row_char_ascii = row_char_ascii + 1
